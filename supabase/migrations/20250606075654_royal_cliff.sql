...
1. User Management - Profiles and preferences
2. Categories - Income and expense categorization  
3. Transactions - Enhanced transaction tracking
4. Financial Targets - Goal setting and progress tracking
5. Dashboard Metrics - Real-time financial overview
6. Payment Methods - Multiple payment options
7. Ledger System - Double-entry bookkeeping
8. Budget Management - Budget planning and tracking
...
-- Create user settings table (for app-specific settings)
CREATE TABLE IF NOT EXISTS user_settings (
  user_id uuid PRIMARY KEY REFERENCES profiles(id) ON DELETE CASCADE,
  monthly_income_target decimal(15,2),
  emergency_fund_goal decimal(15,2),
...
-- Function to sanitize sensitive data in logs
...
-- Function to log events (works with existing audit_logs table)
...
BEGIN
  ...
  DO UPDATE SET
    monthly_income = CASE 
      WHEN TG_OP = 'INSERT' AND NEW.type = 'income' THEN dashboard_metrics.monthly_income + NEW.amount
      WHEN TG_OP = 'UPDATE' AND NEW.type = 'income' AND OLD.type = 'income' THEN dashboard_metrics.monthly_income - OLD.amount + NEW.amount
      WHEN TG_OP = 'UPDATE' AND NEW.type = 'income' AND OLD.type = 'expense' THEN dashboard_metrics.monthly_income + NEW.amount
      WHEN TG_OP = 'UPDATE' AND NEW.type = 'expense' AND OLD.type = 'income' THEN dashboard_metrics.monthly_income - OLD.amount
      WHEN TG_OP = 'DELETE' AND OLD.type = 'income' THEN dashboard_metrics.monthly_income - OLD.amount
      ELSE dashboard_metrics.monthly_income
    END,
    monthly_expenses = CASE 
      WHEN TG_OP = 'INSERT' AND NEW.type = 'expense' THEN dashboard_metrics.monthly_expenses + ABS(NEW.amount)
      WHEN TG_OP = 'UPDATE' AND NEW.type = 'expense' AND OLD.type = 'expense' THEN dashboard_metrics.monthly_expenses - ABS(OLD.amount) + ABS(NEW.amount)
      WHEN TG_OP = 'UPDATE' AND NEW.type = 'expense' AND OLD.type = 'income' THEN dashboard_metrics.monthly_expenses + ABS(NEW.amount)
      WHEN TG_OP = 'UPDATE' AND NEW.type = 'income' AND OLD.type = 'expense' THEN dashboard_metrics.monthly_expenses - ABS(OLD.amount)
      WHEN TG_OP = 'DELETE' AND OLD.type = 'expense' THEN dashboard_metrics.monthly_expenses - ABS(OLD.amount)
